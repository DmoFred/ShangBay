        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr, index) in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }"
                  ...
                  @click="changeChoice(saleAttrValue, saleAttr.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前路由跳转，从A路由跳转B路由，在这加入购物车，进行路由跳转之前，发请求，把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    // 给子组件ZOOM的数据
    skuImageList () {
      // 假如服务器数据没回来，skuInfo这个对象是空对象
      return this.skuInfo.skuImageList || []
    },
  },
  methods: {
    //用户鼠标点击 选择商品属性  例：多大内存 什么颜色 （切换高亮）
    changeChoice (saleAttrValue, arr) {
      // 遍历 全部售卖属性值isChecked为0 为了用户点击时才高亮 默认第一个高亮      
      arr.forEach(item => {
        item.isChecked = '0'
      })
      // 点击后切换高亮
    ...
    changeSkuNum (event) {
      ...
      // 用户输入进来非法情况判断  ↓如果输入的不是数字(NaN)值 或者|| 值小于1 
    ...
    async addCart () {
      ...
      try {
        // 1.发请求----将产品加入到数据库（通知服务器） 成功做什么
        await this.$store.dispatch('addUpdateCart', {
